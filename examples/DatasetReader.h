...
inline int getdir(std::string dir, std::vector<std::string>& files) {
	...
	LOG(INFO) << "files size: " << files.size() << std::endl;
	return files.size();
}
...
private:
	...
	inline void loadTimestampsEUROC() {
		...
		LOG(INFO) << "loading EuRoC time stamps!" << std::endl;
		std::ifstream tr;
		std::string timesFile = path + "/data.csv";
		tr.open(timesFile.c_str());
		if (!tr) {
			LOG(INFO) << "cannot find timestamp file! Something maybe wrong in dataset setting!" << std::endl;
			LOG(INFO) << timesFile << std::endl;
			return;
		}
		...
		while (!tr.eof() && tr.good()) {
			...
			if (2 == sscanf(buf, "%lf,%s", &stamp, filename)) {
				...
				files.push_back(path + "/data/" + std::string(filename));
			}
		}
		tr.close();
		LOG(INFO) << "Load total " << timestamps.size() << " data entries." << std::endl;
	}

	inline void loadTimestampsKitti() {
		LOG(INFO) << "Loading Kitti timestamps!" << std::endl;
		std::ifstream tr;
		std::string timesFile = path + "/times.txt";

		tr.open(timesFile.c_str());
		if (!tr) {
			LOG(INFO) << "cannot find timestamp file at " << path + "/times.txt" << std::endl;
			return;
		}
		...
		for (size_t i = 0; i < timestamps.size(); i++) {
			files.push_back(fmt::format("{}/image_0/{:06d}.png", path, i));
		}
	...
	inline void loadTimestamps() {
		...
		if (!tr) {
			...
			LOG(INFO) << "trying " << timesFile << std::endl;
			tr.close();
			tr.open(timesFile.c_str());
		}
		...
		LOG(INFO) << "files: " << files.size() << " timesstamps: " << timestamps.size() << ", exposures: " << exposures.size() << std::endl;
